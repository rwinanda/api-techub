...
import { addVariantValues, addVariantSKU, editVariantValues, editVariantSKU } from "./variant-values.js";
...
export const createProductVariants = async (
    ...
    // Array for SKU Value
    const tempVar = []; // example = [[128GB, 256GB], [Red, Blue]]

    for(const prodVar of parsedData){
        console.log("Masuk sini?")
        const { product_variant, sku_value } = prodVar; // value

        console.log("Isi productvariant => ", product_variant)
        ...
        for(const variant of product_variant) {
            ...
            try {
                const productVariant = await Database.Data.query(
...
export const editProductVariants = async (parsedData, productId, respProdVar, respSkuValue, respVarValue, req, updated_at) => {
    for(const prodVar of parsedData){
...
export const getProductVariants = async (req, res) => {
    try {
    const product = await Database.Data.query("SELECT * FROM product_variants");
